...
+ function(a) {
    ...
    a.fn.bootstrapToggle = b, a.fn.bootstrapToggle.Constructor = c, a.fn.toggle.noConflict = function() {
    	...
    	for(var aa = 0; aa < a("input[type=checkbox]").length; aa++){
    		if(a(a("input[type=checkbox]")[aa]).prop('checked') == true){
    			a("#row"+a(a("input[type=checkbox]")[aa]).attr('id')).show();
    		}else{
    			a("#row"+a(a("input[type=checkbox]")[aa]).attr('id')).hide();
    		}
    	}
    }), a(document).on("click.bs.toggle", "div[data-toggle^=toggle]", function(b) {
        var c = a(this).find("input[type=checkbox]");
		var d = a(this).find("input[type=checkbox]").attr('alt');

		var dayId=a(this).find("input[type=checkbox]").attr('id');
		var employeeId=$('#employeeId').val();
		var branchId=$('#branchId').val();
		
		if(a(this).find("input[type=checkbox]").prop('checked') == true){
			console.log(d);
			a('#'+d).html('Closed')
			/*alert(a(this).find("input[type=checkbox]").attr('id'));*/
			a("#row"+dayId).hide();
			a('#day'+dayId).hide();
			var isDayOff='Y';
			dayOff(dayId,isDayOff)
		}else{
			a('#'+d).html('Open')
			a("#row"+dayId).show();
			a('#day'+dayId).show();
			//alert(a(this).find("input[type=checkbox]").attr('id')+"hiiii");
			/*alert(a(this).find("input[type=checkbox]").attr('id')+"hiiii");*/
			//call function to day off
			var isDayOff='N';
		   dayOff(dayId,isDayOff)
		}
